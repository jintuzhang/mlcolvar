...
def get_dataset_cv_gradients_pair(
    model: pcvs.PairBaseCV,
    dataset: pdata.PairDataSet,
    component: int = 0,
    device: str = 'cpu',
    ...
    """
    Get gradients of the CV w.r.t. pair distances in a given dataset. The
    calculation will run on the device where the model is on.
    ...
    for batchs in items:
        batch_dict = batchs.to(device).to_dict()
        cv_values, pair_lengths = model(batch_dict, True)
        cv_values = cv_values[:, component]
        grad_outputs = [torch.ones_like(cv_values, device=device)]
        gradients = torch.autograd.grad(
            outputs=[cv_values],
        ...
        n_atoms = dataset.n_atoms_padded
